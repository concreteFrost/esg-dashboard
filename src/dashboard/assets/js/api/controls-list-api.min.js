  $.ajax({ setup,
    success: function (data) {
      getControls(data)
    },
    error: function (e) {
      console.log(e);
    },
  });

function getControls(data){
  $.ajax({
    url: 'https://europe.grc1.com/iGRC/api/Control/Get',
    async: true,
    crossDomain: true,
    method: 'GET',
    data: {
      tenantName: 'ESG One - Production',
    },
    beforeSend: function (xhr) {
      xhr.setRequestHeader('Authorization', 'Bearer ' + data.access_token);
    },

    success: function (data2) {
      const controlsArr = data2;
      $('#controls-datatable').parent().next().remove()
      controlsArr.forEach((c) => {
        const el = `<tr>
        <td>${c.ControlNumber}</td>
        <td><a href="/dashboard/company/individual-control.html#control-overview" onclick="redirectToRisk(this)" data-id=${
          c.ControlId
        }> ${c.ControlName} </a></td>
        <td>${c.Office.OfficeName}</td>
       
        <td>${c.Owner.FullName}</td>
        <td>${c.BusinessUnit.BusinessUnitName}</td>
        <td>${c.ControlCategory ? c.ControlCategory.ControlCategoryName : 'N/A'}</td>
       
        <td>${c.RequiredByRegulator ? "yes" : "no"}</td>
   
        <td>${c.LastReviewDate ?  c.LastReviewDate.split('T')[0] : "N/A" }</td>
        <td>${c.NextReviewDate !== null ? c.NextReviewDate.split('T')[0] : 'N/A'}</td>
        <td>
        
        <button class="btn btn-primary btn-sm" data-toggle="modal"
        data-target="#editControlModal" >edit</button>
        <button class="btn btn-danger btn-sm"onclick="performDeletion(this)" >delete</button></td>
        </tr>`;

        $('#controls-table-body').append(el);
      });

      var control_dt = new DT('#controls-datatable');
    
    },
    error: function (e) {},
  });
}

function performDeletion(e) {
  $(e).closest('tr').remove()
 }

function redirectToControls(e) {
  localStorage.setItem('controls id', $(e).data('id'));
}
