...
function splitByFour(arr) {
  ...
  for (let i = 0; i < arr.length; i += 4) {
    if (arr[i].active) res.push(arr.slice(i, i + 4));
  }
  console.log(arr)
  return res;
}
...
function populateCardTemplate(cardData) {
  ...
  for (let i = 0; i < cardData.length; i++) {
    ...
    setElementText(
      cardData[i].info ? `Info: ${cardData[i].info}` : null,
      cardElement.find('#kpi-card-info')
    );

    setElementText(
      cardData[i].value
        ? `Value: ${cardData[i].value.toLocaleString('en', {
            useGrouping: true,
          })}`
        : null,
      cardElement.find('#kpi-card-value')
    );
    setElementText(
      cardData[i].unit ? `Unit: (${cardData[i].unit})` : null,
      cardElement.find('#kpi-card-unit')
    );
...
function setTargetInfo(
  ...
  value < target
    ? (res = 100 - (value / target) * 100)
    : (res = ((value - target) / target) * 100);
