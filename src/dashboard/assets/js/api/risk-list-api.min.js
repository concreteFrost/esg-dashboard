...
function getRisk(data) {
  $.ajax({
    ...
    success: function (data2) {
      ...
      $('#number-of-risks-red').html(data2.length)
      riskArr.forEach((risk) => {
        ...
        <td><a href="/dashboard/company/individual-risk.html#individual-risk-overview" onclick="redirectToRisk(this)" data-id=${risk.RiskId}> ${risk.RiskName} </a></td>
        ...
        $('#risk-table-body').append(tableElement);

        if(risk.CurrentResidualRiskScore > risk.RegisterTargetScore){
          let alarmedRisk = `<li class="list-group-item">
          <a class="btn btn-secondary w-100" href="/dashboard/company/individual-risk.html#individual-risk-overview" 
          onclick="redirectToRisk(this)" data-id=${risk.RiskId}>${risk.RiskName}</a>
        </li>`

        $('#alarming-risks-list').append(alarmedRisk)
        }
      });
  

      var risk_dt = new DT('#risk-datatable');
