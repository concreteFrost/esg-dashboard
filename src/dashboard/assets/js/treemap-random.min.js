...
*/
var margin={top:0,bottom:0,left:0,right:0},width=$("#card").width()-margin.left-margin.right,height=325,legend=["Environment","Social","Governance"],color=d3.scaleOrdinal().range(["#3C7BC5","#1BA2C1","#5DBFC2"]).domain(legend),tree=d3.treemap().size([width,height]).padding(1).round(!0);
d3.csv("/dashboard/assets/data/treemap-random.csv",function(b,a){a.forEach(function(a){a.value=+a.value1;a.value2=+a.value2});var c=stratifyData(a);tree(c);c=d3.select("#random").selectAll(".node").data(c.leaves(),function(a){return a.data.name}).enter().append("div").attr("class","node");c.append("div").attr("class","node-label");c.append("div").attr("class","node-value");c.call(position)});
...
var svg=d3.select("#random").append("svg").attr("width",width).attr("height",height+70).append("g").attr("class","legend").attr("transform","translate(5, "+(height+40)+")");svg.append("text").style("font-weight","bold").attr("x",0).attr("y",-15).text("Legend");var legendItem=svg.selectAll(".legend-item").data(legend).enter().append("g").attr("class","legend-item").attr("transform",function(b,a){return 2==a?"translate("+120*a+",0)":"translate("+150*a+",0)"});
